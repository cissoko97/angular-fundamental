...
export class DataService implements InMemoryDbService {
  ...
  createDb(reqInfo?: RequestInfo): Record<string, any> | Observable<Record<string, any>> | Promise<Record<string, any>> {
    const developers = USERLIST;
    const geos = GEOS;
    return { developers, geos };
  }

  genId(developers: IUser[]): number {
    return developers.length > 0 ? Math.max(...developers.map(developer => developer.id)) + 1 : 1;
  }
}
